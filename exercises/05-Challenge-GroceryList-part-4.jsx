...
class GroceryList extends React.Component {
  ...
  render() {
    ...
    clearListButton = <button className='clear-list' disabled={this.state.groceries.length==0} onClick={this.clearList}>Clear the List</button>;

    const groceryAddition = <GroceryListAddition addHandler={this.addGroceryItem} />;
...
class GroceryListAddition extends React.Component {
  constructor(props)  {
    super(props);
    this.state = {
      newGroceryName: ""
  ...
  render()  {
    const newProductInput = <input className='new-item' type="text" value={this.state.newGroceryName} onKeyPress={this.onProductInputKeyPress} onChange={this.inputChanged}/>;
    const newProductAddButton = <button className='add-product' disabled={this.state.newGroceryName.trim().length==0} onClick={this.addGroceryItem}>Add new Product</button>;
...
class GroceryListItem extends React.Component {
  ...
  render() {
    ...
    return (
      <li key={this.props.key} className={completed} onClick={this.props.onComplete}>
