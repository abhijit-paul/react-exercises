...
var GroceryList = (function (_React$Component) {
  ...
  _createClass(GroceryList, [{
    ...
    value: function addGroceryItem() {
      if (this.state.newGroceryName.trim().length != 0) {
        this.setState({
    ...
    value: function render() {
      ...
      newProductInput = React.createElement("input", { className: "new-item", value: this.state.newGroceryName, type: "text", onChange: this.inputChanged });
      // Something is missing here... Will anything happen if you click this button now?
      newProductAddButton = React.createElement(
        "button",
        { onClick: this.addGroceryItem, className: "add-product" },
        "Add new Product"
      );
